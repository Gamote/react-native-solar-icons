import * as React from "react";
import Svg, { Circle, Path } from "react-native-svg";
import type { SvgProps } from "react-native-svg";
const SvgDonutBitten = (props: SvgProps) => (
  <Svg
    xmlns="http://www.w3.org/2000/svg"
    width={24}
    height={24}
    fill="none"
    {...props}
  >
    <Circle cx={12} cy={12} r={3} stroke="#000" strokeWidth={1.5} />
    <Path
      stroke="#000"
      strokeLinecap="round"
      strokeWidth={1.5}
      d="M14.5 7 16 5M19 7l1-1M12 5l-1-1M10.5 7l-1.366.366M7 5 6 4M6.792 9.144l-.585-1.288"
    />
    <Path
      fill="#000"
      d="m12.5 22 .034.75a.75.75 0 0 0 .569-1.196zm.444-2.372.446.603zm6.535-4.833-.446-.603zm-4.86 4.06.603-.446zm.7-2.85.446.603zm2.93.165.604-.446zm3.378-1.453-.487.57a.75.75 0 0 0 1.209-.367zm-3.043 1.906-.603.446a.75.75 0 0 0 1.133.084zm.101-.102.53.53a.75.75 0 0 0 .21-.654zm-4.066 2.77-.225.715a.75.75 0 0 0 .755-.185zm.185-.185.53.53a.75.75 0 0 0 .073-.976zM3.021 17.913a.75.75 0 1 0 1.252-.826zm4.943 2.412a.75.75 0 1 0-.656 1.35zM2.75 12A9.25 9.25 0 0 1 12 2.75v-1.5C6.063 1.25 1.25 6.063 1.25 12zM12 2.75A9.25 9.25 0 0 1 21.25 12h1.5c0-5.937-4.813-10.75-10.75-10.75zm1.103 18.804c-.233-.315-.2-.963.287-1.323l-.892-1.206c-1.117.826-1.353 2.404-.601 3.421zm2.12-3.145c-.296-.4-.237-1.224.542-1.8l-.892-1.207c-1.225.907-1.793 2.633-.857 3.9zm.542-1.8c.78-.577 1.586-.392 1.881.007l1.206-.892c-.936-1.265-2.753-1.228-3.979-.322zm4.16-1.211c.496-.367 1-.293 1.215-.11l.973-1.142c-.867-.739-2.162-.633-3.08.046zM21.25 12c0 .872-.12 1.715-.345 2.514l1.444.406c.261-.93.401-1.909.401-2.92zm-8.784 9.25c-.134.007-.264 0-.466 0v1.5c.11 0 .36.007.534 0zm5.18-4.634.335.453 1.206-.892-.335-.453zm1.78-.219c-.047-.274.086-.694.499-.999l-.892-1.206c-.791.585-1.24 1.536-1.087 2.453zm-.312.756.102-.101-1.06-1.061-.102.101zm-5.724 3.078c.357-.265.745-.306 1.004-.225l.45-1.43c-.767-.242-1.648-.068-2.346.449zm.626-.93.185.25 1.206-.891-.185-.25zm1.133.52.186-.185-1.061-1.06-.185.185zM4.273 17.087A9.2 9.2 0 0 1 2.75 12h-1.5c0 2.183.652 4.216 1.771 5.913zM12 21.25a9.2 9.2 0 0 1-4.036-.925l-.656 1.35A10.7 10.7 0 0 0 12 22.75z"
    />
    <Path
      stroke="#000"
      strokeWidth={1.5}
      d="M2 13s2.2 2 4 2c1.212 0 2.606-.908 3.387-1.5M16 15.5c-.5-.5-1.529-.861-2-1.276"
    />
    <Path
      stroke="#000"
      strokeLinecap="round"
      strokeWidth={1.5}
      d="m16.65 8.977.066 1.412M20.678 10.085 19 11.563M5.665 12.641 6.5 11.5M3.683 10.35l-.079-1.412"
    />
  </Svg>
);
export default SvgDonutBitten;
